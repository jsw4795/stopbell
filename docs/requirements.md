...
- Alarm lifecycle transaction은 current lifecycle/activation generation 검증, lifecycle transition, durable logical `NotificationEvent` 생성과 그 시점에 eligible한 Device별 `NotificationDelivery(PENDING)` 생성을 함께 처리한다. commit 뒤 단일 Spring Backend의 fixed-delay, non-overlapping worker는 due PENDING Delivery만 처리한다. 전송 직전 recipient Device의 current owner, enabled 상태, current push target/revision을 다시 확인하고 실제 attempt revision을 기록한다. FCM I/O는 이 transaction 안에서 수행하지 않으며 non-durable after-commit callback만을 유일한 전달 보장으로 사용하지 않는다. claim/lease, `claimedAt`, `SENDING`, stale-claim recovery, multi-worker coordination은 V1 요구가 아니다.
...
- Delivery lifecycle은 `PENDING`, `ACCEPTED`, `FAILED`, `EXPIRED`를 기본 방향으로 하며 retry는 `PENDING + attemptCount + nextAttemptAt + freshness`로 표현할 수 있다. Provider result는 `ACCEPTED`, `INVALID_TARGET`, `RETRYABLE`, `CONFIGURATION`, `PERMANENT_REQUEST`, `AMBIGUOUS_TIMEOUT`을 구분한다. `EXPIRED`는 local freshness 종료 의미이며 Provider accepted는 실제 사용자 표시 성공을 뜻하지 않는다.
